...
while True:
    ...
    try:
        ...
        acc_magHP = signal.filtfilt(b_hp, a_hp, acc_magFilt, padtype='odd', padlen=3*(max(len(b_hp), len(a_hp))-1))
        
        # Mahony 필터로 쿼터니언 업데이트
        gyr = np.array([gyr_x, gyr_y, gyr_z]) * np.pi / 180  # 각속도(rad/s)
        acc = np.array([acc_x, acc_y, acc_z])
        
        # IMU 데이터를 통해 쿼터니언 업데이트
        quat = mahony.updateIMU(q, gyr=gyr, acc=acc)
